...
import { CancelReservation } from "../components/UserDesk/CancelReservation/CancelReservation";
...
describe("Pruebas en componente <CancelReservation />", () => {
  const jsonId = 'bearer';
  const newJson = {bearer: bearer};
  window.localStorage.setItem(jsonId, JSON.stringify(newJson));  
  const wrapper = shallow(<CancelReservation />);
  

  test("Comprobar que CancelReservation renderiza correctamente.", () => {
    expect(wrapper).toMatchSnapshot();
  });

  test("Comprobar que el boton contiene el texto 'Cancelar reserva'.", () => {
    const button = wrapper.find('button').text();
    expect(button).toContain('Cancelar Reserva')
  });
